...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cart']) && ($_POST['action'] ?? '') !== 'void_cart') {
    ...
    try {
        ...
        header('Location: ' . app_url('Sidebar/invoice/receipt.php?sale_id=' . $result['sale_id']));
        exit;
...
const barcodeApiUrl = <?= json_encode(app_url('barcodeScanner/api/barcode.php')) ?>;
const productsApiUrl = <?= json_encode(app_url('barcodeScanner/api/products.php')) ?>;
const heldSalesApiUrl = <?= json_encode(app_url('barcodeScanner/api/held_sales.php')) ?>;
