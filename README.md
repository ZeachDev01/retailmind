...
## Operational workflow update (July 2026)
...
- Segregation of duties: users cannot approve their own replenishment requests or purchase orders.
...
- Inventory insights for days of stock, dead stock, excess stock, expiry risk, and ABC cycle-count scheduling.
- Optional product-family and variant labels, case barcodes, and configurable expiry alert thresholds.

For an existing database, run the idempotent migration after copying the updated files:

```bash
php scripts/migrate.php
```

Then retrain the model so the baseline-comparison metrics appear in Forecast Analytics:

```bash
python demandForcasting/train_model.py
```
...
Build a sanitized production archive with:

```bash
bash scripts/build_release.sh /path/to/inventory_system_release.zip
```
...
3. Run `php scripts/migrate.php` to add the operational workflow tables and columns.
4. Copy `.env.example` to `.env` and enter database, ML API, and email settings.
5. Use the same long `ML_API_KEY` value for the PHP application and the Python service.
6. Install PHP email dependencies:

```bash
composer install --no-dev
```

7. Install Python dependencies. Use the locked versions for repeatable production and CI builds:
...
Migrations are versioned under `database/migrations`. The command reports pending work, applies each migration once, and records it in `schema_migrations`. The **Administration → System Health** page provides a read-only migration and environment status check.
...
The included GitHub Actions workflow also starts MySQL, imports the fresh schema, runs all migrations, executes database integration checks, compiles the Python forecasting code, lints PHP files, and verifies that release ZIPs contain no runtime or sensitive artifacts.
...
`demandForcasting/auto_retrain.py` retrains only when one or more configured conditions are met: missing model, model age, enough new sales records, or WAPE above the threshold.
...
```text
Sidebar/system_administrator/          Administration, users, settings, backups, and system health
barcodeScanner/        Scanner UI and PHP JSON API endpoints
cashier/        Point of sale and cashier dashboard
Sidebar/inventory_management/        Products, inventory, CSV import, counts, replenishment
report/         Forecasts, analytics, readiness, operational reports
demandForcasting/             Random Forest training, API, and automatic retraining
