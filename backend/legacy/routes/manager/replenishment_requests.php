<?php
// manager/replenishment_requests.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_POST['action'] === 'create') {
        ...
        $request_qty = (int)($_POST['request_qty'] ?? 0);
        $notes = trim($_POST['notes'] ?? '');
        $source = ($_POST['source'] ?? '') === 'ml_forecast' ? 'ml_forecast' : 'manual';

        if ($product_id <= 0 || $request_qty <= 0) {
            $error = 'Product and quantity are required';
        } else {
            $stmt = $pdo->prepare("INSERT INTO replenishment_requests (product_id, request_qty, requested_by, source, notes) 
                                   VALUES (?, ?, ?, ?, ?)");
            if ($stmt->execute([$product_id, $request_qty, $_SESSION['user_id'], $source, $notes])) {
                $request_id = (int)$pdo->lastInsertId();
                $message = "Replenishment request #$request_id created successfully";
                log_activity(
                    $pdo,
        ...
        if ($request_id <= 0) {
            ...
            $beforeStmt = $pdo->prepare("SELECT * FROM replenishment_requests WHERE request_id = ?");
            $beforeStmt->execute([$request_id]);
            $before = $beforeStmt->fetch();
            $stmt = $pdo->prepare("UPDATE replenishment_requests SET status = 'approved', approved_by = ?, approved_at = NOW() 
                                   WHERE request_id = ?");
            if ($stmt->execute([$_SESSION['user_id'], $request_id])) {
                $afterStmt = $pdo->prepare("SELECT * FROM replenishment_requests WHERE request_id = ?");
                $afterStmt->execute([$request_id]);
                $after = $afterStmt->fetch();
                $message = "Replenishment request #$request_id approved";
                log_activity($pdo, (int)$_SESSION['user_id'], 'Replenishment approval', 'Replenishment', $request_id, $before, $after);
                $notifyStmt = $pdo->prepare("SELECT u.email, u.user_id, p.product_name, rr.request_qty, COALESCE(np.notify_email,0) AS notify_email, COALESCE(np.notify_inapp,1) AS notify_inapp FROM replenishment_requests rr JOIN users u ON u.user_id = rr.requested_by JOIN products p ON p.product_id = rr.product_id LEFT JOIN notification_preferences np ON np.user_id = u.user_id WHERE rr.request_id = ?");
                $notifyStmt->execute([$request_id]);
                $recipient = $notifyStmt->fetch();
                if ($recipient) {
                    $title = "Replenishment request #{$request_id} approved";
                    $body = "Your request for {$recipient['request_qty']} unit(s) of {$recipient['product_name']} was approved.";
                    if (!empty($recipient['notify_inapp'])) {
                        create_notification($pdo, (int)$recipient['user_id'], 'replenishment', $title, $body, $request_id, 'replenishment');
                    }
        ...
        if ($request_id <= 0) {
            ...
            $beforeStmt->execute([$request_id]);
            $before = $beforeStmt->fetch();
            $stmt = $pdo->prepare("UPDATE replenishment_requests SET status = 'rejected', approved_by = ?, approved_at = NOW() 
                                   WHERE request_id = ?");
            if ($stmt->execute([$_SESSION['user_id'], $request_id])) {
                $afterStmt = $pdo->prepare("SELECT * FROM replenishment_requests WHERE request_id = ?");
                $afterStmt->execute([$request_id]);
                $message = "Replenishment request #$request_id rejected";
                log_activity($pdo, (int)$_SESSION['user_id'], 'Replenishment rejection', 'Replenishment', $request_id, $before, $afterStmt->fetch());
            } else {
                        ...
                        <?php if ($pred['reorder_suggested']): ?>
                            <div class="forecast-item">
                                ...
                                <form method="POST" style="display: inline-block; width: 100%;">
                                    ...
                                    <input type="hidden" name="request_qty" value="<?= $pred['suggested_reorder_qty'] ?? 0 ?>">
                                    ...
                                    <button type="submit" class="btn-add-request">+ Add Request from Random Forest Forecast</button>
                                </form>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                        ...
                        <?php if ($req['status'] === 'pending'): ?>
                            <div class="request-actions">
                                ...
                                <form method="POST" style="display: inline;">
                                    ...
                                    <input type="hidden" name="request_id" value="<?= $req['request_id'] ?>">
                                    <button type="submit" class="btn-action btn-reject" onclick="return confirm('Reject this request?')">Reject</button>
                                </form>
                            </div>
