...
<body><div class="app-shell"><?php include __DIR__ . '/../includes/sidebar.php'; ?><main class="main-content"><div class="topbar"><div><h1>Forecast Analytics</h1><p class="page-subtitle">Backtesting, accuracy trends, feature importance, and product-level performance.</p></div><a class="btn" href="<?= htmlspecialchars(app_url('report/data_readiness.php')) ?>">Data readiness</a></div>
<div class="card-grid"><div class="stat-card"><div class="value"><?= htmlspecialchars(format_ml_metric($metrics, 'mean_absolute_error')) ?></div><div class="label">MAE</div></div><div class="stat-card"><div class="value"><?= htmlspecialchars(format_ml_metric($metrics, 'root_mean_squared_error')) ?></div><div class="label">RMSE</div></div><div class="stat-card"><div class="value"><?= htmlspecialchars(format_ml_metric($metrics, 'wape')) ?><?= isset($metrics['wape']) ? '%' : '' ?></div><div class="label">WAPE</div></div><div class="stat-card"><div class="value"><?= htmlspecialchars(format_ml_metric($metrics, 'smape')) ?><?= isset($metrics['smape']) ? '%' : '' ?></div><div class="label">SMAPE</div></div><div class="stat-card"><div class="value"><?= htmlspecialchars(format_ml_metric($metrics, 'eligible_products', 0)) ?></div><div class="label">Eligible products</div></div><div class="stat-card"><div class="value"><?= htmlspecialchars(format_ml_metric($metrics, 'zero_sales_records', 0)) ?></div><div class="label">Zero-sales dates used</div></div></div>
