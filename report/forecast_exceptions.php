...
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Forecast Exceptions</title><link rel="stylesheet" href="<?=htmlspecialchars(app_url('assets/css/style.css'))?>"><style>.table-wrap{overflow:auto}.reason{display:inline-block;margin:.1rem;padding:.2rem .45rem;border-radius:999px;background:#fee2e2;color:#991b1b;font-size:.78rem}</style></head><body><div class="app-shell"><?php include __DIR__.'/../Sidebar/sidebar.php';?><main class="main-content"><div class="topbar"><div><h1>Forecast Exceptions</h1><p class="page-subtitle">Products requiring human review before AI recommendations become replenishment requests.</p></div><div style="display:flex;gap:.5rem"><a class="btn" href="<?=htmlspecialchars(app_url('report/forecast_analytics.php'))?>">Analytics</a><a class="btn" href="<?=htmlspecialchars(app_url('Sidebar/inventory_management/replenishment_requests.php'))?>">Review recommendations</a></div></div>
