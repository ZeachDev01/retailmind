...
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Demand Forecasts</title><link rel="stylesheet" href="<?= htmlspecialchars(app_url('assets/css/style.css')) ?>"><style>.formula{font-family:ui-monospace,monospace;font-size:.85rem;white-space:nowrap}.range{font-size:.82rem;color:var(--muted)}</style></head>
<body><div class="app-shell"><?php include __DIR__ . '/../Sidebar/sidebar.php'; ?><main class="main-content"><div class="topbar"><div><h1>Random Forest Demand Forecasts</h1><p class="page-subtitle">Forecast ranges, product-level accuracy, and transparent reorder calculations.</p></div><form method="post"><?= csrf_field() ?><button class="btn" name="retrain" value="1">Retrain model</button></form></div>
<?php if ($message): ?><div class="alert <?= htmlspecialchars($messageClass) ?>"><?= htmlspecialchars($message) ?></div><?php endif; ?>
<div style="display:flex;gap:.75rem;flex-wrap:wrap;margin-bottom:1rem"><a class="btn btn-small" href="<?= htmlspecialchars(app_url('report/forecast_analytics.php')) ?>">Forecast analytics</a><a class="btn btn-small" href="<?= htmlspecialchars(app_url('report/data_readiness.php')) ?>">Data readiness</a><?php if (in_array(current_role(), ['admin','super_admin'], true)): ?><a class="btn btn-small" href="<?= htmlspecialchars(app_url('Sidebar/system_administrator/ml_settings.php')) ?>">ML settings</a><?php endif; ?></div>
