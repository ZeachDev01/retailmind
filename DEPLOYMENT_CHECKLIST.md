...
- Run `python -m pip install -r ml/requirements.txt` in a virtual environment.
- Run `python ml/train_model.py` and start `python ml/predict_api.py` using a service manager.
...
- Confirm Apache denies access to `.env`, `ml`, `sql`, `storage`, `scripts`, `config`, and `includes`.
