...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$reset) {
        ...
        $pdo->prepare('UPDATE users SET password_hash = ?, password_changed_at = NOW(), session_version = session_version + 1, failed_login_attempts = 0, locked_until = NULL WHERE user_id = ?')
...
<?php if ($success): ?><div class="alert tag-success"><?= htmlspecialchars($success) ?></div><p><a href="<?= htmlspecialchars(app_url('login.php')) ?>">Continue to login</a></p>
