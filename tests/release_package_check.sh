...
trap 'rm -f "$ZIP"' EXIT
bash "$ROOT/scripts/build_release.sh" "$ZIP" >/dev/null
...
for forbidden in \
  'inventory_system/storage/logs/app.log' \
  'inventory_system/storage/sessions/sess_' \
  'inventory_system/storage/backups/backup_' \
  '.joblib' \
...
for required in \
  'inventory_system/storage/logs/.gitkeep' \
  'inventory_system/storage/sessions/.gitkeep' \
  'inventory_system/storage/backups/.gitkeep' \
  'inventory_system/legacy/demandForcasting/models/.gitkeep'; do
