...
- Run `python -m pip install -r demandForcasting/requirements.txt` in a virtual environment.
- Run `python demandForcasting/train_model.py` and start `python demandForcasting/predict_api.py` using a service manager.
...
- Confirm Apache denies access to `.env`, `demandForcasting`, `sql`, `storage`, `scripts`, `config`, and `includes`.
