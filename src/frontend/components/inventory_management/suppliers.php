...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    try{
        if($action==='save'){
            ...
            $preferred=!empty($_POST['is_preferred'])?1:0;if($preferred){App\Store\StoreWriteGate::execute($pdo,$pdo->prepare('UPDATE supplier_products SET is_preferred=0 WHERE product_id=?'),[$productId]);}
            ...
            App\Store\StoreWriteGate::execute($pdo,$stmt,[$supplierId,$productId,trim((string)($_POST['supplier_sku']??''))?:null,($_POST['last_unit_cost']??'')!==''?(float)$_POST['last_unit_cost']:null,max(1,(int)($_POST['minimum_order_quantity']??1)),max(0,(int)($_POST['lead_time_days']??7)),$preferred]);
            if($preferred){$nameStmt=$pdo->prepare('SELECT supplier_name FROM suppliers WHERE supplier_id=?');$nameStmt->execute([$supplierId]);$name=$nameStmt->fetchColumn();App\Store\StoreWriteGate::execute($pdo,$pdo->prepare('UPDATE products SET preferred_supplier=?,supplier_lead_time_days=? WHERE product_id=?'),[$name,max(0,(int)($_POST['lead_time_days']??7)),$productId]);}
