    ...
    <main class="main-content">
        ...
        <section class="dashboard-section u-mb-15">
            ...
            <div class="attention-list">
                ...
                <?php if (!$needsAttention): ?><div class="empty-state u-empty-min-120"><div><i class="bi bi-check-circle"></i><strong>No urgent inventory issues</strong><span>Stock, expiration, and replenishment indicators are within normal limits.</span></div></div><?php endif; ?>
            </div>
        </section>
        ...
        <div class="dashboard-layout equal">
            <section class="dashboard-section"><div class="section-header"><div><h3>Forecast versus Actual Demand</h3><p class="section-description">Latest 30-day prediction compared with actual units sold.</p></div><a href="<?= htmlspecialchars(app_url('components/report/forecast_analytics.php')) ?>">Analytics</a></div><canvas id="forecastChart" height="145" aria-label="Forecast versus actual demand chart"></canvas></section>
            <section class="dashboard-section"><div class="section-header"><div><h3>Fast-Moving Products</h3><p class="section-description">Top-selling products during the selected period.</p></div></div><div class="chart-list"><?php foreach ($fastMoving as $product): $width = min(100, round(((int)$product['units_sold'] / $maxFastMoving) * 100)); ?><div class="chart-row"><div class="chart-label" title="<?= htmlspecialchars($product['product_name']) ?>"><?= htmlspecialchars($product['product_name']) ?></div><div class="chart-track"><div class="chart-fill" style="width:<?= $width ?>%"></div></div><div class="chart-value"><?= (int)$product['units_sold'] ?> sold</div></div><?php endforeach; ?><?php if (!$fastMoving): ?><div class="empty-state u-empty-min-150"><div><i class="bi bi-bar-chart"></i><strong>No sales in this period</strong><span>Sales activity will appear after checkout transactions.</span></div></div><?php endif; ?></div></section>
        </div>

        <div class="dashboard-layout equal">
            <section class="dashboard-section"><div class="section-header"><div><h3>Low and Out of Stock</h3><p class="section-description">Products requiring monitoring or immediate replenishment.</p></div><a href="<?= htmlspecialchars(app_url('components/inventory_management/products.php?stock=low')) ?>">View all</a></div><div class="table-wrap"><table><thead><tr><th>SKU</th><th>Product</th><th>On Hand</th><th>Reorder</th><th>Action</th></tr></thead><tbody><?php foreach (array_merge($outOfStock, $lowStock) as $product): ?><tr><td><?= htmlspecialchars($product['sku']) ?></td><td><?= htmlspecialchars($product['product_name']) ?></td><td><?= (int)$product['quantity_on_hand'] ?></td><td><?= (int)$product['reorder_level'] ?></td><td><a class="btn btn-small" href="<?= htmlspecialchars(app_url('components/inventory_management/replenishment_requests.php')) ?>"><?= (int)$product['quantity_on_hand'] <= 0 ? 'Restock now' : 'Review' ?></a></td></tr><?php endforeach; ?><?php if (!$lowStock && !$outOfStock): ?><tr><td colspan="5"><div class="empty-state u-empty-min-140"><div><i class="bi bi-check-circle"></i><strong>Stock levels are healthy</strong></div></div></td></tr><?php endif; ?></tbody></table></div></section>
            <section class="dashboard-section"><div class="section-header"><div><h3>Expiring Products</h3><p class="section-description">Remaining batch quantities approaching expiration.</p></div><a href="<?= htmlspecialchars(app_url('components/inventory_management/products.php?stock=expiring')) ?>">View all</a></div><div class="table-wrap"><table><thead><tr><th>Product</th><th>Batch</th><th>Remaining</th><th>Expires</th></tr></thead><tbody><?php foreach ($expiringBatches as $batch): ?><tr><td><?= htmlspecialchars($batch['sku'] . ' — ' . $batch['product_name']) ?></td><td><?= htmlspecialchars($batch['batch_number'] ?? '—') ?></td><td><?= (int)$batch['remaining_quantity'] ?></td><td><?= htmlspecialchars(date('M d, Y', strtotime($batch['expiration_date']))) ?></td></tr><?php endforeach; ?><?php if (!$expiringBatches): ?><tr><td colspan="4"><div class="empty-state u-empty-min-140"><div><i class="bi bi-calendar-check"></i><strong>No batches expiring soon</strong></div></div></td></tr><?php endif; ?></tbody></table></div></section>
        </div>

        <div class="dashboard-layout equal">
            <section class="dashboard-section"><div class="section-header"><div><h3>Pending Replenishment</h3><p class="section-description">Oldest requests waiting for action.</p></div><a href="<?= htmlspecialchars(app_url('components/inventory_management/replenishment_requests.php')) ?>">Manage requests</a></div><div class="indicator-list"><?php foreach ($pendingRequests as $request): ?><div class="indicator-item"><div><strong><?= htmlspecialchars($request['product_name']) ?></strong><span><?= htmlspecialchars($request['sku']) ?> · requested <?= htmlspecialchars(date('M d', strtotime($request['request_date']))) ?></span></div><span class="indicator-value"><?= (int)$request['request_qty'] ?></span></div><?php endforeach; ?><?php if (!$pendingRequests): ?><div class="empty-state u-empty-min-140"><div><i class="bi bi-truck"></i><strong>No pending requests</strong></div></div><?php endif; ?></div></section>
            <section class="dashboard-section"><div class="section-header"><div><h3>Slow-Moving Products</h3><p class="section-description">Products with stock but limited sales during the selected period.</p></div></div><div class="indicator-list"><?php foreach ($slowMoving as $product): ?><div class="indicator-item"><div><strong><?= htmlspecialchars($product['product_name']) ?></strong><span><?= htmlspecialchars($product['sku']) ?> · <?= (int)$product['quantity_on_hand'] ?> on hand</span></div><span class="indicator-value"><?= (int)$product['units_sold'] ?> sold</span></div><?php endforeach; ?><?php if (!$slowMoving): ?><div class="empty-state u-empty-min-140"><div><i class="bi bi-check-circle"></i><strong>No slow-moving products found</strong></div></div><?php endif; ?></div></section>
        </div>
    </main>
</div>
