        ...
        <div class="main-content">
            <header class="page-heading">
                ...
                <div class="page-heading-actions">
                    <form method="GET"><select class="u-select-filter" name="range" onchange="this.form.submit()" aria-label="Dashboard range">
            ...
            <div class="card-grid">
                <a class="stat-card-link" href="<?= htmlspecialchars(app_url('components/cashier/shifts.php')) ?>">
                    <article class="stat-card with-icon <?= $adminMetrics['active_cashiers_today'] > 0 ? 'success' : 'warning' ?>"><span class="stat-icon"><i class="bi bi-person-check-fill"></i></span>
                        ...
                        <div class="stat-meta"><span>Review shifts</span><i class="bi bi-arrow-right"></i></div>
                    </article>
                </a>
                <a class="stat-card-link" href="<?= htmlspecialchars(app_url('components/invoice/receipt.php')) ?>">
                    <article class="stat-card with-icon"><span class="stat-icon"><i class="bi bi-cash-stack"></i></span>
                        <div class="value">₱<?= number_format((float)$adminMetrics['period_sales'], 2) ?></div>
                        <div class="label">Sales — <?= $rangeDays ?> Days</div>
                        <div class="hint">Lifetime: ₱<?= number_format((float)$adminMetrics['total_sales'], 2) ?></div>
                ...
                <a class="stat-card-link" href="<?= htmlspecialchars(app_url('components/modals/audit_log.php')) ?>" data-audit-log-open>
                    <article class="stat-card with-icon"><span class="stat-icon"><i class="bi bi-clock-history"></i></span>
                        ...
                        <div class="hint">Actions in the activity log</div>
                        <div class="stat-meta"><span>Review activity</span><i class="bi bi-arrow-right"></i></div>
                    </article>
                </a>
                <a class="stat-card-link" href="<?= htmlspecialchars(app_url('components/modals/fiscal_periods.php')) ?>" data-fiscal-periods-open>
                    <article class="stat-card with-icon <?= $adminMetrics['open_periods'] > 1 ? 'warning' : 'success' ?>"><span class="stat-icon"><i class="bi bi-calendar-check"></i></span>
                        ...
                        <div class="stat-meta"><span>Manage periods</span><i class="bi bi-arrow-right"></i></div>
                    </article>
                </a>
            </div>
            ...
            <section class="dashboard-section admin-attention-section u-mb-15">
                <div class="section-header">
                    <div>
                        ...
                        <p class="section-description">Items that may require review or compliance action.</p>
                    </div><span class="decision-pill <?= $adminAttentionCount ? 'action' : 'ok' ?>"><?= $adminAttentionCount ? $adminAttentionCount . ' item(s)' : 'All clear' ?></span>
                </div>
            ...
            <div class="dashboard-layout equal">
                <section class="dashboard-section">
                    <div class="section-header">
                        <div>
                            <h3>Sales Trend</h3>
                            <p class="section-description">Daily sales for the selected dashboard range.</p>
                        </div>
                    </div><canvas id="adminSalesChart" height="125"></canvas>
                </section>
                <section class="dashboard-section">
                    <div class="section-header">
                        <div>
                            <h3>Inventory Value by Category</h3>
                            <p class="section-description">Current inventory cost distribution.</p>
                        </div><a href="<?= htmlspecialchars(app_url('components/inventory_management/inventory_overview.php')) ?>">Inventory overview</a>
                    </div><canvas id="adminCategoryChart" height="125"></canvas>
                </section>
            </div>
                    ...
                    <table>
                        ...
                        <?php foreach ($recentTransactions as $tx): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars(format_display_datetime($tx['sale_date'])) ?></td>
                                <td>
                        ...
                        <?php if (!$recentTransactions): ?>
                            <tr>
                                <td class="u-empty-cell" colspan="7">No recent sales recorded yet.</td>
                            </tr>
                        <?php endif; ?>
                    </table>
                </div>
            ...
            new Chart(document.getElementById('adminSalesChart'), {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: c => '₱' + Number(c.raw).toLocaleString(undefined, {
            ...
            new Chart(document.getElementById('adminCategoryChart'), {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: c => `${c.label}: ₱${Number(c.raw).toLocaleString(undefined,{minimumFractionDigits:2})}`
                            }
                        }
                    }
                }
