    ...
    <div class="app-shell"><?php include __DIR__ . '/../sidebar.php'; ?><main class="main-content">
            <div class="topbar">
                <div>
                    ...
                    <p class="page-subtitle">Download portable SQL backups and restore only validated RetailMind backup files.</p>
                </div>
            </div>
            ...
            <div class="card-grid">
                <section class="dashboard-section">
                    <h3>Create backup</h3>
                    <p class="section-description">Exports the table structure and all current records. A history copy is also saved under <code>backend/storage/backups</code>.</p>
                    <form method="post"><?= csrf_field() ?><input type="hidden" name="action" value="backup"><button class="btn">Create and download backup</button></form>
                </section>
                <section class="dashboard-section">
                    <h3>Restore backup</h3>
                    <p class="section-description">A safety backup is created first. Restoring replaces current database tables and records.</p>
                    ...
                    <table>
                        <tr>
                            ...
                            <th>Notes</th>
                                ...
                                <td><?= htmlspecialchars(format_display_datetime($row['created_at'])) ?></td>
                                <td><?= htmlspecialchars($row['filename']) ?></td>
                                <td><?= htmlspecialchars($row['backup_type']) ?></td>
                                <td><?= number_format((int)$row['file_size'] / 1024, 1) ?> KB</td>
                                <td><?= htmlspecialchars($row['status']) ?></td>
                                <td><?= htmlspecialchars($row['full_name'] ?? 'Scheduled task') ?></td>
