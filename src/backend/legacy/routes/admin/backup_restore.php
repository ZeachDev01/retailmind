<?php
// Legacy reachable Database Backup route (ticket #69).
//
// Every action is authorized on its own: both administrator roles may create
// and download a Database Backup, while restoration stays exclusive to the
// Super Administrator. Hiding the restore control is not the protection; this
// guard is.
// The legacy route lives two directories below the backend root, so its shared
// includes are addressed from there. Its broken paths previously made this entry
// point unreachable, which also meant its authorization was never exercised.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'restore') {
        ...
        if (!$confirmed) {
            ...
            $messageClass = 'tag-warning';
        } elseif ((int)$file['size'] > $restorer->supportedUploadBytes()) {
...
<body class="database-backup-page" data-rm-authenticated="true"
    ...
    <div class="app-shell"><?php include __DIR__ . '/../../../../frontend/components/sidebar.php'; ?><main class="main-content">
            <div class="topbar">
                <div>
            ...
            <?php if ($download): ?>
                <section class="dashboard-section backup-ready">
                    <h3>Your backup is ready</h3>
            ...
            <div class="card-grid">
                <section class="dashboard-section">
                    <h3>Create backup</h3>
                    <p class="section-description">Saving changes pauses briefly while a consistent point-in-time copy is captured, then resumes automatically.</p>
                ...
                <?php if (role_capability_policy()->allows($actorRole, RoleCapabilityPolicy::PLATFORM_GOVERNANCE)): ?>
                    <section class="dashboard-section">
                        <h3>Restore backup</h3>
                        <p class="section-description">A safety backup is created first. Protected Audit Records that postdate the snapshot are preserved.</p>
