<?php
// Sidebar/invoice/receipt.php - CRUD interface for receipts
...
function receipt_render_details(array $sale, array $items): void {
    ...
    $receiptUrl = app_url('Sidebar/invoice/receipt.php?sale_id=' . (int)$sale['sale_id']);
    ?>
    <div class="receipt-container">
        ...
        <div class="receipt-actions no-print">
            <a href="<?= htmlspecialchars(app_url('cashier/pos.php')) ?>" class="btn"><i class="bi bi-plus-lg"></i> New Sale</a>
            <button type="button" class="btn btn-secondary" onclick="printReceiptSection(this)"><i class="bi bi-printer"></i> Print Receipt</button>
            <a href="<?= htmlspecialchars(app_url('Sidebar/invoice/receipt.php')) ?>" class="btn btn-secondary"><i class="bi bi-receipt"></i> Receipt History</a>
        </div>
...
<style>
    ...
    .receipt-totals span {
        ...
        .checkout-complete-banner { display:grid;grid-template-columns:auto minmax(0,1fr) auto;gap:1rem;align-items:center;padding:1rem 1.15rem;margin-bottom:1rem;border:1px solid #86efac;border-radius:14px;background:#f0fdf4;color:#166534; }
        .checkout-complete-icon { width:48px;height:48px;display:grid;place-items:center;border-radius:50%;background:#dcfce7;font-size:1.5rem; }
        .checkout-complete-copy strong,.checkout-complete-copy span,.checkout-complete-change span,.checkout-complete-change strong { display:block; }
        .checkout-complete-copy strong { font-size:1.05rem; }
        .checkout-complete-copy span,.checkout-complete-change span { font-size:.8rem;color:#4d7c5b; }
        .checkout-complete-change { text-align:right; }
        .checkout-complete-change strong { font-size:1.45rem; }
        .receipt-actions .btn { display:inline-flex;align-items:center;gap:.4rem; }
        @media(max-width:640px){.checkout-complete-banner{grid-template-columns:auto 1fr}.checkout-complete-change{grid-column:1/-1;text-align:left;padding-top:.65rem;border-top:1px solid #bbf7d0}.receipt-actions{align-items:stretch;flex-direction:column}.receipt-actions .btn{justify-content:center}}
    @media print {
        body {
            background: #fff;
        }
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="actions-cell">
                                    ...
                                    <a href="<?= htmlspecialchars(app_url('Sidebar/invoice/reversals.php?sale_id=' . $receipt['sale_id'])) ?>" class="btn-small btn-edit">Reverse</a>
                                </div>
                            </td>
                        </tr>
                        <?php endforeach; ?>
    ...
    <div id="editModal" class="modal active">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="modal-close" title="Close" onclick="window.location.href='<?= htmlspecialchars(app_url('Sidebar/invoice/receipt.php')) ?>'">&times;</button>
            </div>
            <div class="receipt-container">
                <form method="POST">
                    ...
                    <div style="display: flex; gap: 1rem;">
                        ...
                        <a href="<?= htmlspecialchars(app_url('Sidebar/invoice/receipt.php')) ?>" class="btn">Cancel</a>
                    </div>
                </form>
            </div>
        </div>
...
function viewReceipt(saleId) {
    const url = '<?= htmlspecialchars(app_url('Sidebar/invoice/receipt.php')) ?>?action=view&sale_id=' + saleId + '&ajax=1';
...
async function legacyDeleteReceiptDisabled(saleId) {
    ...
    form.method = 'POST';
    form.action = '<?= htmlspecialchars(app_url('Sidebar/invoice/receipt.php?action=delete&sale_id=')) ?>' + saleId;
